...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($product_name)) {
        ...
        $stmt = $conn->prepare("SELECT id FROM products WHERE seller_id = ? AND sku = ?");
        $stmt->bind_param("is", $seller_id, $sku);
        $stmt->execute();
        $stmt->store_result();
    ...
    if (empty($error_message)) {
        $stmt = $conn->prepare("
            INSERT INTO products (seller_id, product_name, description, category, price, quantity, sku, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("isssidis", $seller_id, $product_name, $description, $category, $price, $quantity, $sku, $status);
        
        if ($stmt->execute()) {
            $success_message = 'Product added successfully!';
            // Redirect to products page
            header('refresh:2;url=seller_products.php');
        } else {
...
<head>
  ...
  <title>Add Product — Seller Dashboard | TradeHub</title>
  ...
  <div class="seller-layout">
    ...
    <aside class="seller-sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li><a href="seller_add_product.php" class="active">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <main class="seller-content">
      ...
      <div style="margin-bottom: 30px;">
        <h1 style="font-size: 28px; font-weight: 700; color: var(--dark); margin-bottom: 8px;">Add New Product</h1>
        <p style="color: var(--text-muted); margin: 0;">Fill in the details below to add a new product to your catalog.</p>
      </div>
      ...
      <div class="form-card">
        ...
        <!-- Product Form -->
        <form method="POST" action="seller_add_product.php">
          ...
          <div class="form-group">
            ...
            <textarea name="description" class="form-control" placeholder="Describe your product..."><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
          </div>
          ...
          <div class="form-group">
            ...
            <select name="status" class="form-control" required>
              <option value="draft" selected>Draft (Not Listed)</option>
          ...
          <div class="button-group">
            <button type="submit" class="btn btn-primary" style="font-size: 15px;">Add Product</button>
